...
1. Once you run the application choose **terraform** template, choose the right authentication method.
...
1. Navigate to the project you created above using **terraform** template.
...
1. Select the **Replace tokens** task.
   ...
   If you observe the **webapp.tf** file in **Exercise 1, Step 3** you will see there are few values are suffixed and prefixed with **\_\_**. For example ****terraformstorageaccount****. Using **Replace tokens** task we will replace those values with the variable values defined in the release pipeline.
